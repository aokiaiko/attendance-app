       ...
       <div class="show-table-wrapper pending-wrapper">
        <table class="show-table">
           ...
           <tr>
              <th>日付</th>
              <td></td>
              <td class="year-cell">{{ $pendingCorrection->attendance->work_date->format('Y年') }}</td>
              <td></td> 
              <td class="date-cell">{{ $pendingCorrection->attendance->work_date->format('n月j日') }}</td>
              <td></td>
              <td></td>
           </tr>
           <tr>
              <th>出勤・退勤</th>
              <td></td>
              <td class="input-cell">{{ $pendingCorrection->requested_clock_in->format('H:i') }}</td>
              <td class="range-cell">～</td>
              <td class="input-cell">{{ $pendingCorrection->requested_clock_out->format('H:i') }}</td>
              <td></td>
              <td></td>
           </tr>
           @foreach ($pendingCorrection->breaks as $index => $break)
            <tr>
            ...
            <tr>
               <th>備考</th>
               <td></td>
               <td colspan="4">{{ $pendingCorrection->note}}</td>
               <td></td>
               <td></td>
            </tr>
        </table>
        ...
        <table class="show-table">
           ...
           <tr>
            ...
            <tr>
                ...
                <td></td>
                <td colspan="3">
                    <textarea class="note-textarea" name="note" >{{ old('note') }}</textarea>
                    @error('note')
                       <div class="input-error">
                          {{ $message }}
                       </div>
